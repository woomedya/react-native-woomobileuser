...
export default class ForgotPassword extends Component {
    ...
    gotoChangePassword = () => {
        pageStore.setPage(pageStore.PAGE_CHANGE_PASSWORD);
    }
    ...
    sendRecovery = async () => {
        ...
        if (this.state.email && !this.state.emailInValidError) {
            try {
                var code = await userApi.sendRecovery(this.state.email);
                if (code == 'ok') {
                    this.setState({
                        serverErrorMessage: ""
                    }, () => {
        ...
        return <KeyboardAvoidingView behavior={Platform.OS == "ios" ? "padding" : "height"} style={{ flex: 1 }}>
            <ScrollView style={{ backgroundColor: '#FFFFFF', flex: 1 }}>
                <View style={{ padding: 30, paddingVertical: 10 }}>
                    ...
                    >{this.state.i18n.forgotpass.title}</Text>
                    <Text
                        style={{ color: '#333333', textAlign: 'center', fontSize: 14, padding: 14, paddingTop: 10 }}
                    >{this.state.i18n.forgotpass.description}</Text>
                </View>
                ...
                <View style={{ padding: 16 }}>
                    ...
                    <View style={{ flexDirection: 'row', paddingVertical: 10 }}>
                        <Button
                            ...
                            title={this.state.i18n.forgotpass.sendcode}
                            type="outline"
                            onPress={this.sendRecovery}
                    ...
                    <View style={{}}>
                        <Button
                            ...
                            title={this.state.i18n.forgotpass.gotcode}
                            type="clear"
